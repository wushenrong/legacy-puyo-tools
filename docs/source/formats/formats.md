...
struct Fpd {
    struct FpdEntry entries[NUM_OF_ENTRIES];
};
```
...
```c
struct Fmp {
    // An unsigned char is used here instead of uint8_t because it is not an
    // actual interger type, but usually on modern systems, uint8_t is a typedef
    // of unsigned char, sizeof(unsigned char) == 1, and CHAR_BIT == 8.
    // This will apply to other types that is not quite an integer.
    unsigned char graphics[NUM_OF_ENTRIES][FONT_WIDTH * FONT_WIDTH / 2];
};
```
...
essentially the `fpd` and `fmp` formats combined. This format has three separate
...
`0xE0 0x03 0xFF 0x7F 0xC6 0x18` then 26 zeros. While the Wii and PSP only
have their identification strings at the end of the file, `GCIX` or `GVRT` for
the Wii and `MIG.00.1PSP` for the PSP. Each character entry is 4 bytes long and
...
struct FntEntry {
    ...
    unsigned char graphic[FONT_WIDTH * FONT_HEIGHT / 2];
};
